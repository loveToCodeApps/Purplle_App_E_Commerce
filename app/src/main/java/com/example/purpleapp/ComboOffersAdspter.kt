...
class ComboOffersAdspter(val data: MutableList<ComboOffersData>) :
    Adapter<ComboOffersViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ComboOffersViewHolder, position: Int) {
        ...
        if (item.img_name == null || item.img_name == "null" || item.img_name == "") {
            ...
            Glide.with(holder.image.context).load(R.drawable.not_available_picture).thumbnail(0.05f)
            ...
            val requestOptions = RequestOptions().diskCacheStrategy(DiskCacheStrategy.ALL)
            Glide.with(holder.image.context).load(item.image).thumbnail(0.05f)
                .apply(requestOptions).into(holder.image)
        ...
        holder.layout.setOnClickListener {
            it.findNavController()
                .navigate(HomeFragmentDirections.actionHomeFragmentToProductDescriptionFragment(item.id))
        }
