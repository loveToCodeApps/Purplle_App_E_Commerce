...
class MyCartAdapter(val data : List<MyCartData>):Adapter<MyCartViewHolder>()
{
    ...
    override fun onBindViewHolder(holder: MyCartViewHolder, position: Int) {
        ...
        holder.img.setImageResource(item.myCartProdImg)
        holder.price.text=item.myCartProdPrice
        holder.cutPrice.text=item.myCartProdCutPrice
        holder.discount.text=item.myCartProdDiscount
        holder.heading.text=item.myCartProDesc
